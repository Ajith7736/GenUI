  ...
  return (
    <div className='bg-light-white/60 sticky top-0 z-40 backdrop-blur-md dark:bg-dark-mediumblack/70 dark:text-dark-white h-[10vh] flex items-center justify-between px-8'>
      ...
      <div className='flex gap-5 xl:gap-8 items-center'>
        ...
        {session ? (<div tabIndex={0} onBlur={() => setTimeout(() => setshowlogout(false), 200)}><Image onClick={() => setshowlogout((prev) => !prev)} src={session?.user?.image ?? '/default-avatar.png'} unoptimized width={40} height={40} alt="Profile" className='w-8 h-8 lg:w-10 lg:h-10 rounded-full cursor-pointer' />
