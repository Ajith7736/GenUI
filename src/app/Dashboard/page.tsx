      ...
      {projecttoggle && !isSubmitted && <div className="dark:bg-dark-black/80  w-[100vw] fixed inset-0 z-20 flex items-end justify-center">
        <div className="bg-light-white py-8 px-5 flex flex-col justify-between  gap-5 bottom-[30%] shadow-md dark:bg-dark-input-outline border dark:border-dark-grey/20 rounded-md h-auto w-[25rem] lg:w-[35rem] absolute z-10">
          <div className="flex justify-between items-center">
            ...
            <div><IoClose className="size-5 cursor-pointer" onClick={handleprojectclose} /></div>
          </div>
          <form action="" className="flex flex-col gap-4" onSubmit={handleSubmit(onSubmit)}>
            <label htmlFor="projectName">Project Name : </label>
            <input type="text" id="projectName" {...register("projectName")} className="dark:bg-dark-input-box border py-1 px-1 dark:border-dark-grey/20 focus:outline-none rounded-md" />
            {errors.projectName && <span className="text-red-500">{errors.projectName.message}</span>}
            <input type="submit" value={isSubmitting ? 'Submitting' : 'Submit'} disabled={isSubmitting} className="p-2 disabled:dark:bg-dark-white/90 dark:bg-dark-white cursor-pointer hover:dark:bg-dark-white/90 transition-all ease-in-out dark:text-dark-black font-bold rounded-md" />
          </form>
        </div>
      </div>}
